...
namespace PblExportTools120
{
    ...
    {
        [DllImport("PBORC120.DLL", EntryPoint = "PBORCA_SessionOpen", CharSet = CharSet.Unicode, SetLastError = true)]
        private static extern int PBORCA_SessionOpen();

        [DllImport("PBORC120.DLL", EntryPoint = "PBORCA_SessionClose", CharSet = CharSet.Unicode, SetLastError = true)]
        private static extern void PBORCA_SessionClose(int hORCASession);

        [DllImport("PBORC120.DLL", EntryPoint = "PBORCA_LibraryDirectory", CharSet = CharSet.Unicode, SetLastError = true)]
        ...
        [DllImport("PBORC120.DLL", EntryPoint = "PBORCA_ConfigureSession", CharSet = CharSet.Auto)]
        private static extern int PBORCA_ConfigureSession(
            int hORCASession,
            PBORCA_CONFIG_SESSION pSessionConfig);

        [DllImport("PBORC120.DLL", EntryPoint = "PBORCA_LibraryEntryExport", CharSet = CharSet.Auto)]
